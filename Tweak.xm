...
%hookf(OSStatus, AudioQueueSetParameter, AudioQueueRef inAQ, AudioQueueParameterID inParamID, AudioQueueParameterValue inValue){
    ...
    if(inParamID==kAudioQueueParam_Volume && inValue){
        // Volume Boost: Clamp high-level volume to 1.0 for stability. 
        // Actual amplification is handled at the PCM level in AudioUnit hooks.
        inValue = MIN(g_curScale, 1.0);
    }
...
%hookf(OSStatus, AudioQueuePrime, AudioQueueRef inAQ, UInt32 inNumberOfFramesToPrepare, UInt32 *outNumberOfFramesPrepared){
    ...
    if(outValue){
        AudioQueueSetParameter(lstAudioQueue, kAudioQueueParam_Volume, MIN(g_curScale, 1.0));
    }
    return %orig;
}
...
- (void)play{
    ...
    if(self.volume){
        self.volume = MIN(g_curScale, 1.0);
    }
    %orig;
}
...
static void setScale(double curScale){
    ...
    // Volume Boost: Sync high-level volumes while keeping them within 0.0-1.0 range
    lstAVAudioPlayer.volume = MIN(g_curScale, 1.0);
    lstAVPlayer.volume = MIN(g_curScale, 1.0);
    [lstAVSampleBufferAudioRenderer setVolume:MIN(g_curScale, 1.0)];
}
...
static void registerApp(){
    ...
    dispatch_once(&onceToken, ^{
        ...
        MRYIPCCenter *idClient = [MRYIPCCenter centerNamed:@"com.vasirakcalgux.volumemixer/register"];

        int pid = NSProcessInfo.processInfo.processIdentifier;
        [idClient callExternalVoidMethod:@selector(register:) withArguments:@{@"bundleID":bundleID, @"pid":@(pid)}];

        NSString *appNotify = [NSString stringWithFormat:@"com.vasirakcalgux.volumemixer/%@~%d/setVolume", bundleID, pid];
...
%ctor{
    ...
    loadPref();
    int token;
    notify_register_dispatch("com.vasirakcalgux.volumemixer/loadPref", &token, dispatch_get_main_queue(), ^(int token) {
        loadPref();
    });
}
