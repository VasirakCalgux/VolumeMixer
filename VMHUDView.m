...
- (void)updateVisuals {
    ...
    if(_curScale > 1.0) {
        ...
        _clippingView.backgroundColor = [UIColor colorWithRed:0.22 green:0.47 blue:0.99 alpha:1.0];
    }

    _clippingView.frame = CGRectMake(0, 
        _clippingView.superview.frame.size.height * (1.0 - _curScale / (_boostEnabled ? 20.0 : 1.0)), 
        _clippingView.frame.size.width, 
        _clippingView.frame.size.height);
}
...
- (void)pan:(UIPanGestureRecognizer *)pan{
    if(pan.state == UIGestureRecognizerStateBegan) {
        ...
        CGFloat maxScale = _boostEnabled ? 20.0 : 1.0;
        CGFloat scale = (1.0 - _clippingView.frame.origin.y / _clippingView.frame.size.height) * maxScale;
        if(scale == _curScale){
            return;
        }
        if(fabs(scale - _curScale) > maxScale / 16.0 || scale <= maxScale / 16.0 || scale == maxScale) {
            _curScale = scale;
...
- (void)changeScale:(CGFloat)dScale {
    CGFloat maxScale = _boostEnabled ? 20.0 : 1.0;
    [self setCurScale:MIN(MAX(_curScale + dScale * maxScale, 0.0), maxScale)];
}
@end
